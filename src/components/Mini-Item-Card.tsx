...
export default function Card({
  ...
  return (
    <main
      className="relative flex max-w-[100px] flex-shrink-0 cursor-pointer flex-col overflow-hidden  rounded-md bg-bg-tertiary-light text-sm transition hover:bg-bg-secondary-light hover:shadow-xs dark:bg-bg-tertiary-dark dark:hover:bg-bg-secondary-dark"
      key={key}
    >
      <Link href={href}>
    ...
    >
      <Link href={href} className="transition-all hover:text-brand-primary">
        <p className="flex w-full items-center justify-center gap-2 p-1 text-center text-xs font-semibold ">
          {rec ? <><ThumbsUp size={17}  /> {textAbove}</>  : formatRelationType()}
        </p>
        <Image
          src={image}
          alt={textBelow && textAbove}
